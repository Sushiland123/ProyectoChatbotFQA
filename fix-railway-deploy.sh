...
git add .
git commit -m "🚂 Fix Railway deployment: Update FAQ and Chatbot services

- Fixed package.json scripts to use start:prod instead of .bat files
- Added railway.toml configuration for both services
- Updated main.ts to use dynamic ports (Railway compatible)
- Added Prisma support in deployment scripts
- Created .env.example files for Railway setup
- Added engines specification for Node.js 18+

Resolves: 'Permission denied' error on Railway Linux servers"
